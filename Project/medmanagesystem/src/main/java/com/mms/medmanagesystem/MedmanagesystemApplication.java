...
public class MedmanagesystemApplication implements CommandLineRunner {
	...
	public void run(String... args) throws Exception { 
		...
		if (pessoaService.getPessoas().isEmpty()) {
			...
			// Internamentos
			internamentoService.saveInternamento(new Internamento(pacienteRepository.getById(1), profissionalRepository.getById(1),(float)0, (new Double[10]), (new Float[10]), (float)0, "Apendicite","2A", EstadoEnum.ESTAVEL.toString(), Date.valueOf("2021-5-20"),Date.valueOf("2022-5-20"), new int[4]));
			internamentoService.saveInternamento(new Internamento(pacienteRepository.getById(2), profissionalRepository.getById(3),(float)0, (new Double[10]), (new Float[10]), (float)0, "Apendicite","2B", EstadoEnum.GRAVE.toString(), Date.valueOf("2021-5-22"),Date.valueOf("2022-5-22"), new int[4]));
